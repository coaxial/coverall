...
Package.prototype.compile = function(done) {
  var self = this;
  self._compileTexToPdf(self.files, function(err) {
    if (err) return done(err);
    done(null);
  });
}
...
Package.prototype._compileTexToPdf = function(tex_files, done) {
  var self = this;
  async.forEachOf(tex_files, function compile(tex_path, filename, next) {
