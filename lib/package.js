...
Package.prototype._mergePdf = function(done) {
  ...
  var output_filename = self.compiled_files.letter.dir + path.sep + changeCase.snakeCase(self.recipient) + '.pdf';
  var letter_path = path.resolve(self.compiled_files.letter.dir + path.sep + self.compiled_files.letter.base);
  var resume_path = path.resolve(self.compiled_files.resume.dir + path.sep + self.compiled_files.resume.base);
  ...
  exec(command, function(err) {
    if (err) return done(err);
    self.compiled_files.package = path.parse(output_filename);
    done(null);
  });
};
