/**
 * @description The Archive class represents a Package that has been compressed and uploaded to the cloud. Every cover
 * letter contains a link to a unique Archive that the recipient can doenload to see the source files and code used to
 * generate their package.
 */
...
/**
 * @constructor
 * @param {Object} pkg - An instance of Package
 * @returns {Object} A new instance of Archive
 */
function Archive(pkg) {
  ...
  self.pkg_long_url = pkg.long_url;
}

/**
 * Fetches all the files to be included in the archive and compresses them. Outputs a tar.gz archive in ./archives/
...
/**
 * @see _uploadToS3
 * @param {Object} options - see _uploadToS3
 * @returns {Function} Promise
 */
Archive.prototype.upload = Promise.method(function(options) {
  var self = this;
  return self._uploadToS3(options);
});
...
/**
 * Makes sure the temporary directory to build the archive exists
 * @returns {Function} Promise
 */
...
/**
 * Moves all the required files to the temporary directory and organizes them into the proper directory tree
 * @returns {Function} Promise
 */
...
/**
 * Compresses all the files into a tar.gz archive and writes it out to ./archives/
 * @returns {Function} Promise
 */
...
/**
 * Removes the temporary directory as it isn't needed anymore after the archive has been built
 * @returns {Function} Promise
 */
 ...
 * @returns {Function} Promise
 */
Archive.prototype._uploadToS3 = function(options) {
  var self = this;
  var archive = fs.createReadStream(self.output_path);
  var file_name = self.name + '.tar.gz';
  var S3Promise = new Promise.promisifyAll(new AWS.S3({
    accessKeyId: options.s3_access_key,
    secretAccessKey: options.s3_secret_key
  }));

  return S3Promise.putObjectAsync({
    Body: archive,
